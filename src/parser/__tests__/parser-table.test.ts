...
describe('MarkdownParser - Tables', () => {
  ...
  describe('snake_case and literal character preservation', () => {
    ...
    it('should not strip asterisks from arithmetic expressions', () => {
      ...
      // remark may parse 100*200 as emphasis; at minimum the cell should exist
    });
  });

  describe('escaped pipe handling', () => {
    it('should treat single \\| as escaped (not a column separator)', () => {
      const md = '| A | B |\n|---|---|\n| cell\\|text | 2 |';
      const result = parser.extractDecorations(md);
      const cells = byType(result, 'tableCell');
      // The \| in cell content should NOT create an extra cell split
      const row2Cells = cells.filter((c) =>
        c.replacement && c.replacement.includes('cell')
      );
      expect(row2Cells.length).toBe(1);
    });
  });
});
